...
class BertWithFGSM(nn.Module):

    def __init__(self, model_path, num_labels, epsilon=0.05, emb_name='word_embeddings'):
        super().__init__()
    ...
    def attack(self):
        ...
        for name, param in self.model.named_parameters():  # 遍历模型的所有参数
            if param.requires_grad and self.emb_name in name:  # 只取word embedding层的参数
                ...
                # 计算扰动，并在输入参数值上添加扰动
                r_at = self.epsilon * np.sign(param.grad.cpu())
                param.data.add_(r_at.to(self.device))
